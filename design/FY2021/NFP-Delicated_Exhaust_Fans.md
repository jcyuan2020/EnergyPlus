...
This work will implement code changes that will allow a more convinient approach to allow exhaust streams from multiple airloops to be re-routed and recombined to one or more general central exhaust systems. A typical example is the dedicated airflow supplied to multple offices and labtories spaces and then required different heat recovery processes for the office spaces exhausts and the lab spaces exhausts. 
...
There could be sizing needs for the cedntral exhaust fan. It could be depends on the sum of the indivdual branches' design flow rates. Further details TBD.
